...
type View = 'home' | 'explore' | 'groups' | 'messages' | 'saved' | 'settings' | 'ai-command-center' | 'personal-ai' | 'story-writer' | 'nearby' | 'group-details' | 'wallet' | 'profile';
  ...
  const [currentUser, setCurrentUser] = useState<CurrentUser | null>(null);
    ...
    const renderMainContent = () => {
        ...
        switch (currentView) {
            ...
            case 'saved': return <SavedView currentUser={currentUser} onReact={handleReact} onComment={handleOpenComments} onSavePost={handleSavePost} onDeletePost={handleDeletePost} userReactions={userReactions} savedPostIds={savedPostIds} onReportPost={handleReportPost} onMuteUser={handleMuteUser} />;
                ...
                <header className="sticky top-0 z-40 w-full border-b bg-background">
                    ...
                    <div className="flex items-center gap-1 sm:gap-2">
                         <div className="hidden lg:flex">
                             ...
                             <Tooltip><TooltipTrigger asChild><Button variant={currentView === 'explore' ? 'secondary' : 'ghost'} size="icon" onClick={() => changeView('explore')}><Compass className="h-6 w-6" /></Button></TooltipTrigger><TooltipContent><p>Trending</p></TooltipContent></Tooltip>
                         ...
                         <Card>
                            <CardContent className="p-2">
                                <nav className="flex flex-col gap-1">
                                    ...
                                    <Button variant={currentView === 'explore' ? 'secondary' : 'ghost'} className="justify-start gap-2" onClick={() => changeView('explore')}><Compass className="h-5 w-5" /> Trending</Button>
                 ...
                 {isClient && currentUser && (
                    <div className="fixed bottom-0 left-0 right-0 z-40 border-t bg-background p-1 lg:hidden">
                        <div className="grid h-full max-w-lg grid-cols-5 mx-auto">
                            ...
                            <Button variant={currentView === 'groups' ? 'secondary' : 'ghost'} className="flex h-auto flex-col gap-1 rounded-md p-2" onClick={() => changeView('groups')}>
