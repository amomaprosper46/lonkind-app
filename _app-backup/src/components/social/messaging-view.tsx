...
import { Search, Send, MessageSquare, Loader2, Copy, Check, Mic, Square, Trash2, ArrowLeft } from 'lucide-react';
...
export default function MessagingView({ initialConversationId, currentUser }: MessagingViewProps) {
    ...
    useEffect(() => {
        ...
        const q = query(conversationsRef, where('participantUids', 'array-contains', user.uid), orderBy('lastMessage.timestamp', 'desc'));

        const unsubscribe = onSnapshot(q, async (querySnapshot) => {
             const convos: Conversation[] = querySnapshot.docs.map(doc => {
                const data = doc.data();
                return {
                    id: doc.id,
                    participants: data.participants,
                    participantUids: data.participantUids,
                    lastMessage: data.lastMessage || { text: 'No messages yet', type: 'text', timestamp: null },
                } as Conversation;
            });
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            await updateDoc(doc(db, 'conversations', selectedConversation.id), { lastMessage: messageData });


            const otherParticipant = getOtherParticipant(selectedConversation);
            if (otherParticipant) {
                await addDoc(collection(db, 'users', otherParticipant.uid, 'notifications'), {
                    type: 'new_message',
                    fromUser: { 
                        uid: currentUser.uid, 
    ...
    if(loadingAuth){
        return (
            <div className="col-span-12 md:col-span-9 flex items-center justify-center p-8">
    ...
    return (
        <main className="col-span-12 md:col-span-9">
            <Card className="h-[calc(100vh-12rem)] md:h-[calc(100vh-10rem)] flex">
                {/* Conversation List */}
                <div className={cn("w-full md:w-1/3 border-r flex-col", (isMobile && selectedConversation) ? "hidden" : "flex")}>
                ...
                {/* Chat Window */}
                <div className={cn("w-full md:w-2/3 flex-col bg-secondary/30", (isMobile && !selectedConversation) ? "hidden" : "flex")}>
                    {selectedConversation ? (
                        <>
