...
import { doc, getDoc, collection, query, where, orderBy, onSnapshot, addDoc, serverTimestamp, updateDoc, arrayUnion, arrayRemove, increment, writeBatch } from 'firebase/firestore';
...
export default function GroupDetailsView({ 
    ...
    const handleCreatePost = async () => {
        ...
        try {
            ...
            const newPostDoc = await addDoc(collection(db, 'posts'), postData);
            
            // Fan-out notifications to group members
            const membersToNotify = group.members.filter(uid => uid !== currentUser.uid);
            if (membersToNotify.length > 0) {
                const batch = writeBatch(db);
                membersToNotify.forEach(memberId => {
                    const notificationRef = doc(collection(db, 'users', memberId, 'notifications'));
