...
async function creditCoinsForPayment(reference, userId, coinAmount, amountNaira) {
...
exports.paystackWebhook = (0, https_1.onRequest)(async (req, res) => {
    ...
    const expectedHash = (0, crypto_1.createHmac)('sha512', PAYSTACK_SECRET_KEY)
        .update(rawBody)
        .digest('hex');
    let signatureValid = false;
    try {
        signatureValid = (0, crypto_1.timingSafeEqual)(Buffer.from(expectedHash, 'hex'), Buffer.from(signature, 'hex'));
    }
    catch (e) {
    ...
    try {
        if (event.event === 'charge.success') {
            ...
            if (userId && coinAmount && reference) {
                await creditCoinsForPayment(reference, userId, Number(coinAmount), amount / 100);
