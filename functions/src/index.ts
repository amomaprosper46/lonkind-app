...
/**
 * Manual Trigger Instance (HTTP Endpoint for Admin Testing)
 */
export const manualNewsReporterTrigger = onRequest(
  {
    secrets: ["GEMINI_API_KEY", "SERPER_API_KEY"],
    memory: "512MiB",
    timeoutSeconds: 120,
  ...
  async (req, res) => {
    logger.info("⚡ Manual Trigger: Executing autonomous news reporter worker...");
    try {
      const result = await autonomousNewsReporter();
      logger.info(`✅ Success! Posted update to timeline. Firestore Post ID: ${result.postId}`);
      res.status(200).json({ success: true, message: "News Reporter ran successfully", postId: result.postId });
    } catch (error: any) {
      logger.error("❌ Manual function run crashed:", error.message);
...
async function creditCoinsForFlwPayment(
  reference: string,
  userId: string,
  coinAmount: number,
  amount: number,
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (existing.exists) {
      logger.info(`[Flw Webhook] Reference ${reference} already processed — skipping`);
      return;
    }
...
async function handleFlwTransferEvent(event: any) {
  const transfer = event.data;
  const reference = transfer.reference as string;
  const transferId = transfer.id;

  let payoutSnap = await db.collection("payoutRequests")
    .where("reference", "==", reference)
    .limit(1)
    .get();

  if (payoutSnap.empty && transferId) {
    payoutSnap = await db.collection("payoutRequests")
  ...
  if (transfer.status === "SUCCESSFUL") {
    await payoutRef.update({ status: "completed", flwStatus: "SUCCESSFUL" });
    ...
    await db.runTransaction(async (transaction) => {
      transaction.update(payoutRef, { status: "failed", flwStatus: transfer.status });

      const userRef = db.collection("users").doc(payoutData.userId);
      if (payoutData.diamondAmount) {
        transaction.update(userRef, {
          diamonds: FieldValue.increment(payoutData.diamondAmount),
        });
      }
    });
    ...
    await db.collection("users").doc(payoutData.userId)
      .collection("notifications").add({
        ...
        amount: payoutData.amount,
        currency: payoutData.currency || "NGN",
        reason: transfer.complete_message || transfer.reason || "Transfer failed",
        timestamp: FieldValue.serverTimestamp(),
        read: false,
      });
  }
...
export const flutterwaveWebhook = onRequest(
  async (req, res) => {
    const FLW_SECRET_HASH = process.env.FLW_SECRET_HASH || process.env.FLUTTERWAVE_SECRET_HASH;
    if (!FLW_SECRET_HASH) {
      logger.error("FLW_SECRET_HASH is not configured in environment secrets.");
      res.status(500).send("Server configuration error.");
      return;
    }
    ...
    if (!verifHash || verifHash !== FLW_SECRET_HASH) {
      logger.warn("[Flw Webhook] Invalid or missing verif-hash header — block attempt");
      res.status(401).send("Unauthorized");
      return;
    }

    const event = req.body;
    logger.info(`[Flw Webhook] Processing valid event type: ${event.event || event["event.type"]}`);

    try {
      const eventType = event.event || event["event.type"];
      if (eventType === "charge.completed" || eventType === "charge.successful") {
        const { meta, amount, tx_ref, currency } = event.data || {};
        const { userId, coinAmount } = meta || {};

        if (userId && coinAmount && tx_ref) {
          await creditCoinsForFlwPayment(
            tx_ref,
            userId,
            Number(coinAmount),
            amount,
          ...
          logger.info(`[Flw Webhook] ✅ Successfully processed charge tx_ref: ${tx_ref}`);
        } else {
          logger.warn("[Flw Webhook] charge payload missing metadata properties", meta);
        }
      } else if (["transfer.completed", "transfer.failed", "transfer.reversed"].includes(eventType)) {
        await handleFlwTransferEvent(event);
        logger.info(`[Flw Webhook] ✅ Handled transfer ledger update: ${eventType}`);
      } else {
        logger.info(`[Flw Webhook] Event hash verified but type unhandled: ${eventType}`);
      }
    } catch (error) {
      logger.error("[Flw Webhook] Execution crash inside event processing block:", error);
    }
